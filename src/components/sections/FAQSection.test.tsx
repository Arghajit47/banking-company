...
describe("FAQSection", () => {
  ...
  // frames (390, 1440, 1920) for FAQ body copy, so this is a single value and
  ...
  it("subheading body copy renders 150% line-height, not leading-relaxed", () => {
    ...
    render(<FAQSection />);
    const el = screen.getByTestId("faq-subheading");
  ...
  it("every FAQ answer renders 150% line-height, not leading-relaxed", () => {
    mockUseFAQConfig.mockReturnValue(
      baseReturn({ faqs: DEFAULT_FAQS, hasMore: false })
    );
    render(<FAQSection />);
